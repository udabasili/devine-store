...
export const getAllClothings = () => {
  ...
  let items = Object.values(clothing).map((clothingValue) => (
    clothingValue.items.map((item: any) => (
      item
  ))))
...
const filterFunction = (value: Filter) => {
  ...
  if (value.price) {
    filteredClothings = filteredClothings.filter(clothing => {
      if ((clothing.price >= value.price[0]) && (clothing.price <= value.price[1])) {
        return clothing
      }
      return false
    })
...
export default function Provider ({ children }: ProviderProps) {
  ...
  return (
    <Context.Provider value={{
      ...
      setCurrentUser: (value: object | User ) => dispatch({type: 'SET_USER', payload: value}),
      setShowDropDown: (value: boolean ) => dispatch({type: 'DROPDOWN_TOGGLE', payload: value}),
      setFilter: (value: Filter) => dispatch({type: 'FILTER_DATA', payload: value}),
